...
**Hand-authored pages carrying their own `<script defer src="/assets/consent.js">` (7, not 5 — easy to undercount):** `docs/index.html`, `docs/privacy-policy.html`, `docs/terms-of-service.html`, `docs/impressum.html`, `docs/404.html`, `docs/join.html`, `docs/scan/android-qr/index.html`. Any new hand-authored page needs the same `<script>` tag or it silently keeps loading nothing (fails closed) rather than gating GA correctly on that page.
