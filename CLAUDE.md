...
- **New blog post:** write it in **both languages** — `content/blog/<slug>.md` and a full German translation at `content/de/blog/<slug>.md`, each with `blogIndex: true` and bidirectional `altPath` (same as every existing post; there is no EN-only path). FAQ under `## Frequently asked questions` / `## Häufige Fragen` auto-becomes FAQPage JSON-LD. Run build, then add the URL to `docs/llms.txt` **as a Markdown link** (`- [Title](url): note`) under the `## Blog` section — a bare URL fails Lighthouse's Agentic Browsing audit ("file contains no links").
