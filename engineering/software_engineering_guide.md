...
- **Keychain accessibility (iOS), v1.37.2.** The auth Keychain items are written with `SECURE_STORE_OPTIONS` = `AFTER_FIRST_UNLOCK` (`packages/api/src/storage.ts`), **never** expo-secure-store's default `WHEN_UNLOCKED` — the default throws `errSecInteractionNotAllowed` ("User interaction is not allowed") when auth-js's auto-refresh ticker touches the Keychain on a backgrounded, locked device (Sentry `REACT-NATIVE-M`). The storage adapter never throws and serves a last-known-good value on a failed read. **A failed Keychain read is not a sign-out** — `readStoredSessionResult()` returns `{ session: null, storageUnavailable: true }` and every consumer (`offlineWindowState`, `useAuthInit` boot + `SIGNED_OUT` handler) keeps the app open. The auto-refresh ticker is gated on app foreground via `useSupabaseAutoRefresh` (`AppState`-driven `startAutoRefresh`/`stopAutoRefresh`). Installed users are re-keyed once by `keychainAccessibilityMigration.ts` (delete + re-add — `setItemAsync` on an existing key does NOT re-apply `kSecAttrAccessible`).
