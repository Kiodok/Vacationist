...
export function CreateRentalSheet({ visible, onClose, onSubmit, isPending, currency, tripStartDate, tripEndDate }: CreateRentalSheetProps) {
  ...
  const { control, handleSubmit, reset, watch, setValue, formState: { errors } } = useForm<CreateTransferRentalInput>({
    resolver: zodResolver(createTransferRentalSchema),
    defaultValues: { title: '', currency: initialTransferCurrency(currency), is_business: false },
  });
  ...
  const resetValues = () => ({ title: '', currency: initialTransferCurrency(currency), is_business: false });
          ...
          <View className="bg-surface-elevated rounded-t-lg px-md pt-md max-h-[90%]" style={{ paddingBottom: Math.max(insets.bottom, 32) }}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              <View className="gap-md">
                ...
                {/* Price */}
                <View className="gap-xs">
                  <Text className="text-label text-text-muted uppercase">{t('rental.field.price')} ({currencyField.currencySymbol})</Text>
                ...
                <Controller
                  ...
                  render={({ field: { onChange, value } }) => (
                    <View className="flex-row items-center justify-between py-xs">
                      <Text className="text-body text-text-primary">{t('rental.field.businessExpense')}</Text>
                      <Switch
                        value={value ?? false}
