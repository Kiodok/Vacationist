...
interface EditPublicTransportSheetProps {
  visible: boolean;
  onClose: () => void;
  onSubmit: (input: UpdateTransferPublicTransportInput) => void;
  isPending: boolean;
...
export function EditPublicTransportSheet({ visible, onClose, onSubmit, isPending, entry, currency, tripStartDate, tripEndDate }: EditPublicTransportSheetProps) {
  ...
  useEffect(() => {
    if (visible) {
      reset({
        ...
        departure_location: entry.departure_location ?? undefined,
        arrival_location: entry.arrival_location ?? undefined,
        departure_time: entry.departure_time ?? undefined,
      ...
      setPriceText(entry.price_total != null ? String(entry.price_total) : '');
    }
  }, [visible, entry]);

  const onValid = (data: UpdateTransferPublicTransportInput) => {
    Keyboard.dismiss();
    onSubmit(data);
  };
        ...
        <View className="flex-1 justify-end">
          ...
          <View className="bg-surface-elevated rounded-t-lg px-md pt-md max-h-[90%]" style={{ paddingBottom: Math.max(insets.bottom, 32) }}>
            ...
            <View className="flex-row items-center justify-between mb-md">
              <Text className="text-heading-m text-text-primary">{t('publicTransport.edit.title')}</Text>
