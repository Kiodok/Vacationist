...
export function EmptyPublicTransport() {
  ...
  return (
    <View className="flex-1 items-center justify-center gap-sm py-xl">
      <View className="w-[72px] h-[72px] rounded-full bg-warning-muted items-center justify-center">
        <ThemedIcon name="train-outline" size={32} color={colors.warning} />
      </View>
      <Text className="text-heading-s text-text-secondary">{t('publicTransport.empty.title')}</Text>
      <Text className="text-body-small text-text-muted text-center px-xl">
        {t('publicTransport.empty.subtitle')}
      </Text>
      <Pressable
      ...
      <InfoSheet
        ...
        title={t('publicTransport.info.title')}
        content={t('publicTransport.info.content')}
      />
    </View>
  );
}
