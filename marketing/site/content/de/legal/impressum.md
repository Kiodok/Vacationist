---
title: Impressum — Vacationist
description: Impressum und Anbieterkennzeichnung nach Schweizer Recht für die Vacationist-App und -Website.
path: /de/impressum/
lang: de
type: legal
schema: WebPage
date: 2026-07-21
breadcrumbLabel: Impressum
---

# Impressum

*Anbieterkennzeichnung nach Schweizer Recht (UWG Art. 3 Abs. 1 lit. s & nDSG Art. 19) · Zuletzt aktualisiert: Juli 2026*

## 1. Verantwortliche Person

Diese Website und die Vacationist-Mobil-App werden als persönliches Nebenprojekt (Nebenerwerb, nicht gewinnorientiert) betrieben von:

| | |
|---|---|
| **Name** | Gary Lude |
| **Strasse** | Kirchstrasse 12 |
| **PLZ / Ort** | 8573 Alterswilen |
| **Kanton** | Thurgau |
| **Land** | Schweiz |
| **E-Mail** | [meetdeep.de@gmail.com](mailto:meetdeep.de@gmail.com) |
| **UID** | Keine — Privatperson; keine kommerzielle Registrierung |
| **Handelsregister** | Nicht eingetragen |
| **MWST** | Nicht registriert (Jahresumsatz unter der Schwelle von CHF 100'000) |

> Dieses Projekt wird als nicht gewinnorientierte Nebenerwerbstätigkeit einer natürlichen Person betrieben. Es werden keine Einnahmen erzielt. Es handelt sich nicht um ein eingetragenes Unternehmen; es besteht kein Handelsregistereintrag und keine UID-Nummer.

## 2. Angaben zur Anwendung

| | |
|---|---|
| **App-Name** | Vacationist |
| **Plattform** | Android (Google Play Store) |
| **Paket-ID** | com.vacationist.mobile |
| **Website** | [vacationist.app](https://vacationist.app/) |

## 3. Rechtsrahmen & Gerichtsstand

Die Pflicht zur Führung dieses Impressums ergibt sich aus **Art. 3 Abs. 1 lit. s des Bundesgesetzes gegen den unlauteren Wettbewerb (UWG)**, der irreführende oder vorenthaltene Identitätsangaben in kommerzieller Kommunikation untersagt.

Dieses Impressum und die zugehörigen Bedingungen unterliegen im Übrigen Schweizer Recht, insbesondere dem Obligationenrecht (OR), dem Bundesgesetz über den Datenschutz (nDSG, in Kraft seit 1. September 2023) und — soweit EU-Ansässige betroffen sind — der EU-Datenschutz-Grundverordnung (DSGVO).

Für Streitigkeiten aus der Nutzung dieser Website oder der Vacationist-App sind ausschließlich die zuständigen Schweizer Gerichte am Wohnsitz des Betreibers zuständig, soweit nicht zwingende gesetzliche Bestimmungen etwas anderes vorschreiben.

## 4. Haftungsausschluss

Der Betreiber übernimmt keine Gewähr für die Richtigkeit, Vollständigkeit oder Aktualität der Inhalte dieser Website. Links zu externen Websites dienen nur der Information; für die Inhalte verlinkter externer Seiten ist der Betreiber nicht verantwortlich.

Die Vacationist-App wird kostenlos und „wie besehen" ohne jegliche Gewährleistung bereitgestellt. Der vollständige Haftungsausschluss steht in den [Nutzungsbedingungen](/de/terms-of-service/).

## 5. Datenschutz

Gemäß **Art. 19 nDSG** ist die verantwortliche Person verpflichtet, betroffene Personen über die Bearbeitung ihrer Personendaten zu informieren. Alle Angaben dazu, welche Daten erhoben werden, zu den Bearbeitungszwecken, Aufbewahrungsfristen und Betroffenenrechten stehen in der [Datenschutzerklärung](/de/privacy-policy/).

Die Aufsichtsbehörde für Datenschutzfragen in der Schweiz ist der Eidgenössische Datenschutz- und Öffentlichkeitsbeauftragte (EDÖB): [www.edoeb.admin.ch](https://www.edoeb.admin.ch).

Betroffene Personen, die ihre Rechte nach nDSG oder DSGVO verletzt sehen, können zusätzlich Beschwerde bei der Aufsichtsbehörde des EU-Mitgliedstaats ihres gewöhnlichen Aufenthalts, Arbeitsplatzes oder des Orts des mutmaßlichen Verstoßes einreichen (DSGVO Art. 77).

## 6. Streitbeilegung

Der Betreiber ist nicht verpflichtet und grundsätzlich nicht bereit, an Streitbeilegungsverfahren vor einer Verbraucherschlichtungsstelle teilzunehmen. In der Schweiz können sich Verbraucher in Zivilsachen an die zuständige kantonale Schlichtungsbehörde nach der Schweizerischen Zivilprozessordnung (ZPO) wenden.

Nur zur Information: EU-Verbraucher können auch die Online-Streitbeilegungsplattform (OS) der Europäischen Kommission nutzen: [ec.europa.eu/consumers/odr](https://ec.europa.eu/consumers/odr). Der Betreiber sitzt in der Schweiz (Nicht-EU) und ist rechtlich nicht zur Teilnahme an EU-Streitbeilegungsverfahren verpflichtet, wird angemessene Anfragen an [meetdeep.de@gmail.com](mailto:meetdeep.de@gmail.com) aber prüfen.

## 7. Urheberrecht

Alle Inhalte dieser Website und der Vacationist-App — einschließlich, aber nicht beschränkt auf Design, Code, Logos und Texte — sind geistiges Eigentum von Gary Lude, soweit nicht anders angegeben. Vervielfältigung oder Verbreitung ohne vorherige schriftliche Zustimmung ist nicht gestattet.
