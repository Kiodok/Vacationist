---
url: https://vacationist.app/de/privacy-policy/
...
Vacationist erhebt nur die Daten, die für die Reiseplanung nötig sind, speichert sie auf Supabase-Infrastruktur in der Schweiz (Zürich) und verschlüsselt Reisedokumente zusätzlich mit AES-256 — diese Seite erklärt im Detail, welche Daten das sind und wie du sie löschen kannst.
...
Neben Google Analytics und dem Reddit-Pixel betreiben wir eine kleine, eigene Analyse-Erfassung auf unserer eigenen Infrastruktur, die Website-Seitenaufrufe, Klicks auf die Play-Store-/App-Store-/Web-App-Links sowie App-Anmeldungen erfasst. In der Web-Version der App (web.vacationist.app) erfasst sie ausserdem, wenn du eine Reise erstellst, einen Einladungslink erzeugst, einer Reise beitrittst oder eine Ausgabe hinzufügst. Sie existiert nur, damit wir an einem Ort sehen können, wie die App entdeckt und zum ersten Mal genutzt wird, unabhängig von Google und Reddit.

Sie speichert bewusst weniger als ein typisches Analyse-Tool: **es wird niemals eine IP-Adresse gespeichert**, weder vollständig noch gekürzt. Wo eine grobe Besucherkennung nötig ist, um denselben Besuch nicht doppelt zu zählen, berechnen wir einen Einweg-Hash aus deiner IP-Adresse, deinem Browser und einem geheimen, täglich wechselnden Salt — deine IP-Adresse selbst wird nur für diesen Moment verwendet und niemals in eine Datenbank geschrieben. Wir speichern: den Seitenpfad, die Reddit-Klick-Kennung und Kampagnenparameter, falls vorhanden, die Domain der verweisenden Seite (nicht die vollständige URL), den User-Agent-String deines Browsers sowie — bei Anmeldungen und den oben genannten Aktionen in der Web-App — nur, dass die Aktion stattgefunden hat, verknüpft mit deinem Konto, ohne weitere Details (kein Reisename, keine eingeladene Person, kein Betrag). Nichts von dem, was du eingibst oder planst, wird hier erfasst.
