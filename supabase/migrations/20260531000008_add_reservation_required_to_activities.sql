-- Add reservation_required flag to activities.
-- Allows trip members to mark activities that need advance booking.
-- Defaults to FALSE so existing rows are unaffected.

ALTER TABLE public.activities
  ADD COLUMN reservation_required BOOLEAN NOT NULL DEFAULT FALSE;

-- Recreate create_activity RPC to accept the new column.
-- DROP required because PostgreSQL forbids changing the signature via CREATE OR REPLACE.
...
BEGIN
  ...
  INSERT INTO public.activities (
    ...
    reservation_required, created_by
  )
  VALUES (
    ...
    p_reservation_required, v_caller
  )
