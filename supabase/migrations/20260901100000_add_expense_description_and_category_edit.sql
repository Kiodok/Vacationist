-- v1.33.0: expense description field (task 7) + category becomes editable (task 6)
--
-- Why: `expenses` had no description column at all — add one, editable on both create and
-- update. `update_expense_with_splits` never accepted `related_type`, so the category picked at
-- creation could never be changed afterward — add `p_related_type` the same way `p_currency`
-- was added in 20260809100006/20260809110000: a trailing DEFAULT NULL parameter, with NULL
-- meaning "keep the existing value" so an already-running (pre-update) app instance that omits
-- the parameter entirely can't accidentally blank out a value it doesn't know about yet.
--
-- `p_description` uses the same NULL-means-keep-existing convention, but since a description is
-- a genuinely clearable free-text field (unlike related_type, which always has one of five fixed
-- values), an explicit empty string is treated as "clear it" — NULL and '' are deliberately
-- different signals here.
--
-- Both RPC bodies below are copied verbatim from the current live versions
-- (20260809100006_update_expense_rpcs_fx.sql for create_expense_with_splits,
-- 20260809110000_backward_compat_update_expense_rpc.sql for update_expense_with_splits) with
-- only the description/related_type additions inserted — every existing validation is preserved
-- unchanged. update_expense_with_splits gains two new trailing parameters, which changes its
-- argument-type signature, so the old 7-arg overload must be dropped first (CREATE OR REPLACE
-- does not replace a function with a different argument list — it creates an overload, and two
-- resolvable overloads for the same PostgREST call is the ambiguity bug fixed on 2026-08-09).
...
----------------------------------------------------------------------
-- 2. create_expense_with_splits — new trailing p_description param
----------------------------------------------------------------------
...
BEGIN
  ...
  INSERT INTO public.expenses (trip_id, title, description, amount, currency, paid_by, related_type, related_id, split_method, created_by, exchange_rate, converted_amount)
  VALUES (
    p_trip_id, p_title,
...
----------------------------------------------------------------------
-- 3. update_expense_with_splits — new trailing p_related_type / p_description params
----------------------------------------------------------------------

DROP FUNCTION IF EXISTS public.update_expense_with_splits(UUID, TEXT, NUMERIC, UUID, TEXT, JSONB, TEXT);
