-- v1.39.0 items 8 + 13: six new expense categories, and a separate "tip" amount on expenses.
--
-- Categories: the expense "category" IS expenses.related_type (related_id is never populated by the
-- app, so it already behaves as a pure category). The set grows from 5 to 11:
--   food_drink, groceries, fuel_parking, tickets_entry, health, souvenirs
-- The value list is enforced in THREE places, all updated here: the table CHECK constraint and the
-- two hard-coded `NOT IN (...)` guards inside create_expense_with_splits / update_expense_with_splits
-- (20260901150000). Extending only the CHECK would let the table accept a category the RPCs then
-- reject — and every expense write goes through those RPCs.
--
-- Tip: `amount` keeps its existing meaning — the GRAND TOTAL that is split between members and
-- converted (converted_amount) — and `tip_amount` records how much of that total was tip. Because
-- amount is untouched, FX, split math, balances and the cost-summary RPCs need no change. The tip is
-- split with the bill, so it is deliberately metadata only.
--   * create_expense_with_splits gains a trailing `p_tip_amount NUMERIC DEFAULT 0`.
--   * update_expense_with_splits gains a trailing `p_tip_amount NUMERIC DEFAULT NULL`, following the
--     NULL-means-keep-existing convention of p_related_type / p_description / p_is_business
--     (20260901100000, 20260901150000) so an already-running old app instance cannot reset a tip it
--     does not know about. In that legacy case the kept tip is clamped to the new amount
--     (LEAST) — otherwise an old client lowering an amount below the stored tip would fail the
--     CHECK with an opaque error, for a value it cannot even see.
--
-- Both functions gain a trailing parameter, which changes the argument-type signature, so the old
-- overloads are dropped first (same as 20260809110001, 20260901100001): CREATE OR REPLACE does not
-- replace a function with a different argument list, it creates an ambiguous second overload.

----------------------------------------------------------------------
-- 1. Table: category CHECK + tip_amount
----------------------------------------------------------------------

-- The CHECK was declared inline in 20260513100001 (unnamed). Postgres would auto-name it
-- `expenses_related_type_check`, but rather than trust that (a wrong name makes DROP ... IF EXISTS a
-- silent no-op and leaves the old 5-value constraint blocking every new category), find whatever
-- CHECK on public.expenses mentions related_type and drop that.
DO $$
DECLARE
  v_con RECORD;
BEGIN
  ...
  LOOP
    EXECUTE format('ALTER TABLE public.expenses DROP CONSTRAINT %I', v_con.conname);
  END LOOP;
END;
$$;

ALTER TABLE public.expenses ADD CONSTRAINT expenses_related_type_check CHECK (
  related_type IN (
...
-- Table-level (it compares two columns). Existing rows all have tip_amount = 0 and amount > 0, so
-- adding it validates instantly against the current data.
ALTER TABLE public.expenses
  ADD CONSTRAINT expenses_tip_amount_range CHECK (tip_amount >= 0 AND tip_amount <= amount);

----------------------------------------------------------------------
-- 2. create_expense_with_splits — new trailing p_tip_amount param + 11-value category guard
----------------------------------------------------------------------

DROP FUNCTION IF EXISTS public.create_expense_with_splits(UUID, TEXT, NUMERIC, TEXT, UUID, TEXT, UUID, TEXT, JSONB, TEXT, BOOLEAN);
...
BEGIN
  ...
  INSERT INTO public.expenses (trip_id, title, description, amount, currency, paid_by, related_type, related_id, split_method, created_by, exchange_rate, converted_amount, is_business, tip_amount)
  VALUES (
    p_trip_id, p_title,
...
----------------------------------------------------------------------
-- 3. update_expense_with_splits — new trailing p_tip_amount param + 11-value category guard
----------------------------------------------------------------------

DROP FUNCTION IF EXISTS public.update_expense_with_splits(UUID, TEXT, NUMERIC, UUID, TEXT, JSONB, TEXT, TEXT, TEXT, BOOLEAN);
