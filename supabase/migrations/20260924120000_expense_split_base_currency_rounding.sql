-- v1.39.1: foreign-currency expenses left a stray cent (or two) in the base-currency splits.
--
-- Root cause (confirmed on prod, trip "Sarajevo", BAM @ 1.95583): create/update_expense_with_splits
-- freeze converted_amount = ROUND(p_amount * rate, 2) ONCE, but convert each split independently with
-- ROUND(v_amount * rate, 2). Sum-of-roundings != rounding-of-sum:
--   Einkauf 14.09.   78.46 + 0 + 39.23 + 39.21 = 156.90   vs converted 156.92   (-0.02)
--   Spritgeld Dodge  60.64 + 0 + 30.32 + 30.32 = 121.28   vs converted 121.26   (+0.02)
--   Muflon           17.11 + 0 + 34.22 + 17.11 =  68.44   vs converted  68.45   (-0.01)
-- That residue is what left a fully settled trip at a +0.01 net balance (see 20260924100000). Same-
-- currency expenses (rate = 1) never drift, so this only ever affected multi-currency trips.
--
-- Fix, going forward only: after the splits are written, private.absorb_split_rounding() adds the
-- residue to ONE split so SUM(amount_owed) = converted_amount exactly — the same "one share absorbs the
-- remainder" rule the expense-currency amounts already follow (packages/utils/src/expenseSplits.ts).
--   * it picks the LARGEST non-sentinel split, so a real member's share moves by at most a cent or two
--     and a zero share can never be pushed below the amount_owed >= 0 CHECK;
--   * it NEVER touches the "Deleted User" sentinel split — a departed member's historical share must
--     not be silently rewritten (deleted-member-expense-split skill);
--   * 'cover' is exempt — it assigns the full amount to the covered member by design, so its splits
--     intentionally do not sum to the total;
--   * a residue larger than 0.50 is not rounding (50 splits can drift at most ~0.25) and is left alone.
-- amount_owed_original_currency (what the user typed) is untouched. Historical rows keep their drift;
-- the reminder predicate no longer depends on it.
--
-- The two RPC bodies below are the live 20260922100000 versions, verbatim, plus one PERFORM each.
-- CREATE OR REPLACE with identical signatures — no DROP needed, grants preserved.

CREATE OR REPLACE FUNCTION private.absorb_split_rounding(
  p_expense_id    UUID,
  p_converted_amt NUMERIC,
  p_split_method  TEXT
)
RETURNS VOID
LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT p_converted_amt - COALESCE(SUM(amount_owed), 0)
    INTO v_drift
    FROM public.expense_splits
   WHERE expense_id = p_expense_id;

  IF v_drift = 0 OR ABS(v_drift) > 0.50 THEN
    RETURN;
  END IF;
  ...
  SELECT id INTO v_target
   ...
   WHERE expense_id = p_expense_id
     ...
     AND amount_owed + v_drift >= 0
   ORDER BY amount_owed DESC, id
   LIMIT 1;
  ...
  UPDATE public.expense_splits
     SET amount_owed = amount_owed + v_drift
   WHERE id = v_target;
END;
$$;
