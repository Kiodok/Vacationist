-- Add 'cover' split method: payer covers the full expense for exactly one other person.

----------------------------------------------------------------------
-- 1. Update CHECK constraint on expenses.split_method
----------------------------------------------------------------------
...
BEGIN
  SELECT conname INTO v_constraint_name
   ...
   WHERE conrelid = 'public.expenses'::regclass
     AND contype = 'c'
     AND pg_get_constraintdef(oid) LIKE '%split_method%';
...
ALTER TABLE public.expenses
  ...
  CHECK (split_method IN ('even', 'exact', 'shares', 'cover'));

----------------------------------------------------------------------
-- 2. Update create_expense_with_splits
----------------------------------------------------------------------
